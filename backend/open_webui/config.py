...
ENABLE_NVIDIA_API = PersistentConfig(
    "ENABLE_NVIDIA_API",
    "nvidia.enable",
    os.environ.get("ENABLE_NVIDIA_API", "True").lower() == "true",
)

NVIDIA_API_KEY = os.environ.get("NVIDIA_API_KEY", "")
NVIDIA_API_BASE_URL = os.environ.get("NVIDIA_API_BASE_URL", "https://integrate.api.nvidia.com/v1")

if NVIDIA_API_BASE_URL.endswith("/"):
    NVIDIA_API_BASE_URL = NVIDIA_API_BASE_URL[:-1]

NVIDIA_API_KEYS = os.environ.get("NVIDIA_API_KEYS", "")
NVIDIA_API_KEYS = NVIDIA_API_KEYS if NVIDIA_API_KEYS != "" else NVIDIA_API_KEY

NVIDIA_API_KEYS = [key.strip() for key in NVIDIA_API_KEYS.split(";") if key.strip()]
NVIDIA_API_KEYS = PersistentConfig(
    "NVIDIA_API_KEYS", "nvidia.api_keys", NVIDIA_API_KEYS
)

NVIDIA_API_BASE_URLS = os.environ.get("NVIDIA_API_BASE_URLS", "")
NVIDIA_API_BASE_URLS = (
    NVIDIA_API_BASE_URLS if NVIDIA_API_BASE_URLS != "" else NVIDIA_API_BASE_URL
)

NVIDIA_API_BASE_URLS = [
    url.strip() if url != "" else "https://integrate.api.nvidia.com/v1"
    for url in NVIDIA_API_BASE_URLS.split(";")
]
NVIDIA_API_BASE_URLS = PersistentConfig(
    "NVIDIA_API_BASE_URLS", "nvidia.base_urls", NVIDIA_API_BASE_URLS
)

NVIDIA_API_CONFIGS = PersistentConfig(
    "NVIDIA_API_CONFIGS",
    "nvidia.api_configs",
    {},
)
