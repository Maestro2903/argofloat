...
from open_webui.routers import openai, ollama, nvidia
...
async def fetch_nvidia_models(request: Request, user: UserModel = None):
    try:
        raw_nvidia_models = await nvidia.get_nvidia_models(request, user=user)
        return [
            {
                "id": model["id"],
                "name": model["name"],
            ...
            for model in raw_nvidia_models.get("models", [])
        ]
    except Exception as e:
        log.exception(f"Error fetching NVIDIA models: {e}")
        return []  # Return empty list on error to prevent breaking the app
...
async def get_all_base_models(request: Request, user: UserModel = None):
    ...
    nvidia_task = (
        fetch_nvidia_models(request, user)
        if request.app.state.config.ENABLE_NVIDIA_API
        else asyncio.sleep(0, result=[])
    )
