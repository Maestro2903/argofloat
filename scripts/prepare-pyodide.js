...
async function downloadPackages() {
	...
	// Check if we already have a valid cache with lock file
	try {
		const lockFile = await readFile('static/pyodide/pyodide-lock.json', 'utf8');
		...
		if (lockFile && pyodideVersion === pyodidePackageVersion) {
			console.log('✓ Pyodide cache found and valid, skipping download');
			return;
		}
	} catch (err) {
		console.log('No valid Pyodide cache found, proceeding with setup...');
	}
