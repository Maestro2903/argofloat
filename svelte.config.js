...
const config = {
	...
	kit: {
		...
		version: {
			name: (() => {
				// Use environment variable if available (set by CI/CD)
				...
				try {
					return child_process.execSync('git rev-parse HEAD').toString().trim().slice(0, 7);
				} catch {
	...
	onwarn: (warning, handler) => {
		...
		if (code === 'unused-export-let') return;
		if (code === 'export_let_unused') return; // Svelte 5 compatibility
